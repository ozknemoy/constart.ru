  ...
  return {
    'link': function (scope, element, attrs) {
      ...
      var resize = function () {
        ...
        // resize the image
          // отнимаю ширину сайдбара и соразмерно высоту   if(w>768) это точка слома
          console.log("calc",windowWidth);
          var heightDiff = sideBar*imageDisplayDimensions.height/imageDisplayDimensions.width;

        element.css({
          'width': windowWidth>768 ? imageDisplayDimensions.width - sideBar + 'px' : imageDisplayDimensions.width + 'px',
          'height': windowWidth>768 ? imageDisplayDimensions.height - heightDiff + 'px' : imageDisplayDimensions.height + 'px'
        });
        ...
        // default width of 600px and that places .modal-content off center
          // тут тоже отнимаю высоту
          // пока закомменттил высоту modal-content
        /*angular.element(
          document.querySelector('.lightbox-modal .modal-content')
        ).css({
          'height': formatDimension(modalDimensions.height- heightDiff)
        });*/
      };
